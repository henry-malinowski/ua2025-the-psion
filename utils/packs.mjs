...
function cleanPackEntry(data, { clearSourceId = true, ownership = 0 } = {}) {
  ...
  if (data._stats?.lastModifiedBy)
    data._stats.lastModifiedBy = "dnd5ebuilder0000";
 ...
 *
 * - `bun run build:clean` - Clean all source files.
 * - `bun run build:clean -- classes` - Only clean the source files for the specified compendium.
 * - `bun run build:clean -- classes Barbarian` - Only clean a single item from the specified compendium.
 */
 ...
 *
 * - `bun run build:db` - Compile all files into their LevelDB files.
 * - `bun run build:db -- classes` - Only compile the specified pack.
 */
 ...
 *
 * - `bun run build:source` - Extract all compendium LevelDB files into source files.
 * - `bun run build:source -- classes` - Only extract the contents of the specified compendium.
 * - `bun run build:source -- classes Barbarian` - Only extract a single item from the specified compendium.
 */
    ...
    await extractPack(packInfo.path, dest, {
      ...
      transformName: (entry) => {
        if (entry._id in folders)
          return path.join(folders[entry._id].path, "_folder.yml");
        if (entry._id in containers)
          return path.join(containers[entry._id].path, "_container.yml");
